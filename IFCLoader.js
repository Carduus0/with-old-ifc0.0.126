...
class IFCParser {
  ...
  async newIfcModel(buffer) {
    ...
    this.currentWebIfcID = await this.state.api.OpenModel(
      data,
  ...
  async initializeLoadingState(modelID) {
    const shapes = await this.state.api.GetLineIDsWithType(
      modelID,
      IFCPRODUCTDEFINITIONSHAPE
    );
  ...
  updateLoadingState() {
    ...
    if (realCurrentItem / this.loadingState.total >= this.loadingState.step) {
      const currentProgress = Math.ceil(
        this.loadingState.total * this.loadingState.step
      );
  ...
  streamMesh(modelID, mesh) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      let itemMesh = this.getPlacedGeometry(
        modelID,
        mesh.expressID,
        placedGeometry
      );
...
class ItemsMap {
  ...
  fillItemsWithGroupInfo(group, geometry, items) {
    ...
    for (let i = materialStart; i <= materialEnd; i++) {
      ...
      if (isEndOfMaterial) {
        const store = this.getMaterialStore(
          items.map,
          expressID,
          materialIndex
        );
...
class SubsetCreator {
  ...
  insertNewIndices(config, subsetID, materialIndex, newIndices) {
    ...
    if (!config.material) {
      this.insertIndicesAtGroup(
        subsetID,
        indicesOfOneMaterial,
        materialIndex,
        newIndices
      );
    } else {
...
class SubsetManager {
  constructor(state, BVH) {
    ...
    this.BVH = BVH;
    this.subsetCreator = new SubsetCreator(
      state,
      this.items,
      this.subsets,
      this.BVH
  ...
  createSubset(config) {
    const subsetID = this.getSubsetID(
      config.modelID,
      config.material,
      config.customID
    );
...
class BasePropertyManager {
  ...
  getRelated(rel, propNames, IDs) {
    ...
    if (!element) {
      return console.warn(
        `The object with ID ${rel.expressID} has a broken reference.`
      );
    }
...
class WebIfcPropertyManager extends BasePropertyManager {
  ...
  async getSpatialStructure(modelID, includeProperties) {
    ...
    const allLines = await this.state.api.GetLineIDsWithType(
      modelID,
      IFCPROJECT
    );
  ...
  async getProperty(modelID, elementID, recursive = false, propName) {
    const propSetIds = await this.getAllRelatedItemsOfType(
      modelID,
      elementID,
      propName
    );
    ...
    for (let i = 0; i < propSetIds.length; i++) {
      result.push(
        await this.state.api.GetLine(modelID, propSetIds[i], recursive)
      );
    }
    return result;
  }
  ...
  async getChunks(modelID, chunks, propNames) {
    const relation = await this.state.api.GetLineIDsWithType(
      modelID,
      propNames.name
    );
  ...
  async getAllRelatedItemsOfType(modelID, id, propNames) {
    const lines = await this.state.api.GetLineIDsWithType(
      modelID,
      propNames.name
    );
    const IDs = [];
...
class JSONPropertyManager extends BasePropertyManager {
  ...
  async getSpatialStructure(modelID, includeProperties) {
    const chunks = await this.getSpatialTreeChunks(modelID);
    const projectsIDs = await this.getAllItemsOfType(
      modelID,
      IFCPROJECT,
      false
    );
  ...
  async getProperty(modelID, elementID, recursive = false, propName) {
    const resultIDs = await this.getAllRelatedItemsOfType(
      modelID,
      elementID,
      propName
    );
    ...
    if (recursive) {
      result.forEach((result) =>
        this.getReferencesRecursively(modelID, result)
      );
    }
    return result;
  }
  ...
  async getChunks(modelID, chunks, propNames) {
    const relation = await this.getAllItemsOfType(
      modelID,
      propNames.name,
      true
    );
...
class PropertySerializer {
  ...
  async getItemProperty(modelID, id, properties) {
    try {
      ...
      console.log(
        `There was a problem getting the properties of the item with ID ${id}`
      );
    }
  }
  ...
  async getBuilding(modelID) {
    const allBuildingsIDs = await this.webIfc.GetLineIDsWithType(
      modelID,
      IFCBUILDING
    );
...
class PropertyManager {
  ...
  async getMaterialsProperties(modelID, elementID, recursive = false) {
    this.updateCurrentProps();
    return this.currentProps.getMaterialsProperties(
      modelID,
      elementID,
      recursive
    );
  ...
  async getSpatialStructure(modelID, includeProperties) {
    ...
    return await this.currentProps.getSpatialStructure(
      modelID,
      includeProperties
    );
  }
...
class FlatMeshVector {
  constructor(serializer, vector) {
    ...
    const keys = Object.keys(vector)
      ...
      .map((key) => parseInt(key));
    keys.forEach(
      (key) => (this._data[key] = serializer.reconstructFlatMesh(vector[key]))
    );
  }
...
class SerializedGeometry {
  constructor(geometry) {
    var _a, _b, _c, _d;
    this.position =
      ((_a = geometry.attributes.position) === null || _a === void 0
        ? void 0
        : _a.array) || [];
    this.normal =
      ((_b = geometry.attributes.normal) === null || _b === void 0
        ? void 0
        : _b.array) || [];
    this.expressID =
      ((_c = geometry.attributes.expressID) === null || _c === void 0
        ? void 0
        : _c.array) || [];
    this.index =
      ((_d = geometry.index) === null || _d === void 0 ? void 0 : _d.array) ||
      [];
...
class GeometryReconstructor {
  static new(serialized) {
    ...
    GeometryReconstructor.set(
      geom,
      "expressID",
      new Uint32Array(serialized.expressID),
      1
    );
    GeometryReconstructor.set(
      geom,
      "position",
      new Float32Array(serialized.position),
      3
    );
    GeometryReconstructor.set(
      geom,
      "normal",
      new Float32Array(serialized.normal),
      3
    );
...
class PropertyHandler {
  ...
  getMaterialsProperties(modelID, elementID, recursive) {
    return this.handler.request(
      this.API,
      WorkerActions.getMaterialsProperties,
      {
        modelID,
        elementID,
        recursive,
...
class WebIfcHandler {
  ...
  async SetGeometryTransformation(modelID, transformationMatrix) {
    return this.handler.request(
      this.API,
      WorkerActions.SetGeometryTransformation,
      {
        modelID,
  ...
  async StreamAllMeshesWithTypes(modelID, types, meshCallback) {
    ...
    return this.handler.request(
      this.API,
      WorkerActions.StreamAllMeshesWithTypes,
      {
        modelID,
        types,
      }
...
class WorkerStateHandler {
  ...
  async updateStateWebIfcSettings() {
    ...
    return this.handler.request(
      this.API,
      WorkerActions.updateStateWebIfcSettings,
      {
        webIfcSettings,
      }
    );
  ...
  async updateModelStateJsonData(modelID, jsonData) {
    return this.handler.request(
      this.API,
      WorkerActions.updateModelStateJsonData,
      {
        modelID,
        jsonData,
      }
  ...
  async loadJsonDataFromWorker(modelID, path) {
    return this.handler.request(
      this.API,
      WorkerActions.loadJsonDataFromWorker,
      {
        modelID,
        path,
      }
...
(function (DBOperation) {
  ...
  DBOperation[(DBOperation["transferIndividualItems"] = 1)] =
...
class ParserHandler {
  ...
  async setupOptionalCategories(config) {
    ...
    return this.handler.request(
      this.API,
      WorkerActions.setupOptionalCategories,
      {
        config,
      }
    );
  ...
  resolveCallbacks(data) {
    if (this.callbackHandlers[data.id]) {
      ...
      if (this.callbackHandlers[data.id].serializer) {
        callbackParameter = this.callbackHandlers[data.id].serializer(
          data.result
        );
      }
...
class IFCUtils {
  ...
  isA(entity, entity_class) {
    ...
    if (entity_class) {
      if (
        this.state.api.GetNameFromTypeCode(entity.type) ===
        entity_class.toUpperCase()
      ) {
        test = true;
      }
      return test;
  ...
  async idsByType(modelID, entity_class) {
    let entities_ids = await this.state.api.GetLineIDsWithType(
      modelID,
      Number(
        this.state.api.GetTypeCodeFromName(modelID, entity_class.toUpperCase())
      )
    );
    return entities_ids;
  }
...
class Data {
    ...
    for (let i = 0; i < workSchedules.length; i++) {
      ...
      this.workSchedules[workSchedule.expressID] = {
        ...
        Description: workSchedule.Description
          ? workSchedule.Description.value
          : "",
        Creators: [],
        CreationDate: workSchedule.CreationDate
          ? workSchedule.CreationDate.value
          : "",
        StartTime: workSchedule.StartTime ? workSchedule.StartTime.value : "",
        FinishTime: workSchedule.FinishTime
          ? workSchedule.FinishTime.value
          : "",
        TotalFloat: workSchedule.TotalFloat
          ? workSchedule.TotalFloat.value
          : "",
        RelatedObjects: [],
      };
    }
  ...
  async loadWorkScheduleRelatedObjects(modelID) {
    let relsControls = await this.utils.byType(
      modelID,
      "IfcRelAssignsToControl"
    );
    for (let i = 0; i < relsControls.length; i++) {
      let relControls = relsControls[i];
      let relatingControl = await this.utils.byId(
        modelID,
        relControls.RelatingControl.value
      );
      let relatedObjects = relControls.RelatedObjects;
      if (this.utils.isA(relatingControl, "IfcWorkSchedule")) {
        for (
          var objectIndex = 0;
          objectIndex < relatedObjects.length;
          objectIndex++
        ) {
          this.workSchedules[relatingControl.expressID]["RelatedObjects"].push(
            relatedObjects[objectIndex].value
          );
        }
      }
    }
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      this.tasks[task.expressID] = {
        ...
        TaskTime: task.TaskTime
          ? await this.utils.byId(modelID, task.TaskTime.value)
          : "",
  ...
  async loadTaskSequence(modelID) {
    ...
    for (let i = 0; i < relsSequence.size(); i++) {
      ...
      if (relSequenceId !== 0) {
        ...
        this.tasks[relatingProcess]["IsPredecessorTo"].push(
          relSequence.expressID
        );
        this.tasks[related_process]["IsSucessorFrom"].push(
          relSequence.expressID
        );
      }
    }
  ...
  async loadTaskOutputs(modelID) {
    let rels_assigns_to_product = await this.utils.byType(
      modelID,
      "IfcRelAssignsToProduct"
    );
    for (let i = 0; i < rels_assigns_to_product.length; i++) {
      let relAssignsToProduct = rels_assigns_to_product[i];
      let relatedObject = await this.utils.byId(
        modelID,
        relAssignsToProduct.RelatedObjects[0].value
      );
      if (this.utils.isA(relatedObject, "IfcTask")) {
        let relatingProduct = await this.utils.byId(
          modelID,
          relAssignsToProduct.RelatingProduct.value
        );
        this.tasks[relatedObject.expressID]["Outputs"].push(
          relatingProduct.expressID
        );
      }
    }
  }
  ...
  async loadTaskNesting(modelID) {
    ...
    for (let i = 0; i < rels_nests.length; i++) {
      ...
      let relating_object = await this.utils.byId(
        modelID,
        relNests.RelatingObject.value
      );
      if (this.utils.isA(relating_object, "IfcTask")) {
        let relatedObjects = relNests.RelatedObjects;
        for (
          var object_index = 0;
          object_index < relatedObjects.length;
          object_index++
        ) {
          this.tasks[relating_object.expressID]["IsNestedBy"].push(
            relatedObjects[object_index].value
          );
          this.tasks[relatedObjects[object_index].value]["Nests"].push(
            relating_object.expressID
          );
        }
      }
    }
  ...
  async loadTaskOperations(modelID) {
    let relsAssignsToProcess = await this.utils.byType(
      modelID,
      "IfcRelAssignsToProcess"
    );
    for (let i = 0; i < relsAssignsToProcess.length; i++) {
      let relAssignToProcess = relsAssignsToProcess[i];
      let relatingProcess = await this.utils.byId(
        modelID,
        relAssignToProcess.RelatingProcess.value
      );
      if (this.utils.isA(relatingProcess, "IfcTask")) {
        ...
        for (
          ...
          this.tasks[relatingProcess.expressID]["OperatesOn"].push(
            relatedObjects[object_index].value
          );
        }
      }
  ...
  async loadAssignementsWorkCalendar(modelID) {
    let relsAssignsToControl = await this.utils.byType(
      modelID,
      "IfcRelAssignsToControl"
    );
    for (let i = 0; i < relsAssignsToControl.length; i++) {
      ...
      let relatingControl = await this.utils.byId(
        modelID,
        relAssignsToControl.RelatingControl.value
      );
      if (this.utils.isA(relatingControl, "IfcWorkCalendar")) {
        ...
        for (
          ...
          this.tasks[relatedObjects[object_index].value][
            "HasAssignmentsWorkCalendars"
          ].push(relatingControl.expressID);
        }
      }
    }
  }
    ...
    for (let i = 0; i < workCalendars.length; i++) {
      ...
      let workCalenderData = {
        ...
        Description: workCalendar.Description
          ? workCalendar.Description.value
          : "",
        WorkingTimes: workCalendar.WorkingTimes
          ? workCalendar.WorkingTimes
          : [],
        ExceptionTimes: workCalendar.ExceptionTimes
          ? workCalendar.ExceptionTimes
          : [],
      };
  ...
  async loadWorkTimes(modelID) {
    ...
    for (let i = 0; i < workTimes.length; i++) {
      ...
      let workTimeData = {
        ...
        RecurrencePattern: workTime.RecurrencePattern
          ? await this.utils.byId(modelID, workTime.RecurrencePattern.value)
          : "",
    ...
    for (let i = 0; i < timePeriods.length; i++) {
      ...
      let workTimeData = {
        StartTime: timePeriod.StartTime
          ? new Date(timePeriod.StartTime.value)
          : "",
...
class IFCManager {
  ...
  async parse(buffer) {
    ...
    let model = await this.parser.parse(
      buffer,
      (_a = this.state.coordinationMatrix) === null || _a === void 0
        ? void 0
        : _a.toArray()
    );
  ...
  async useJSONData(useJSON = true) {
    ...
    if (useJSON) {
      await ((_a = this.worker) === null || _a === void 0
        ? void 0
        : _a.workerState.updateStateUseJson());
    }
  }
  ...
  async addModelJSONData(modelID, data) {
    ...
    if (this.state.worker.active) {
      await ((_a = this.worker) === null || _a === void 0
        ? void 0
        : _a.workerState.updateModelStateJsonData(modelID, data));
    } else {
      model.jsonData = data;
    }
  }
  ...
  async loadJsonDataFromWorker(modelID, path) {
    ...
    if (this.state.worker.active) {
      await ((_a = this.worker) === null || _a === void 0
        ? void 0
        : _a.workerState.loadJsonDataFromWorker(modelID, path));
    }
  }
  ...
  async initializeWorkers() {
    ...
    await this.worker.parser.setupOptionalCategories(
      this.parser.optionalCategories
    );
